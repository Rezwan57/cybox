...
const HomePage = ({ setUrl }: { setUrl: (url: string) => void }) => (
  <div className="p-8 text-white">
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      <button onClick={() => setUrl('cybox://bank')} className="bg-neutral-700 p-4 rounded-lg hover:bg-green-700 transition-colors">
        <p className="font-bold">CyberBank</p>
        <p className="text-sm text-neutral-300">Secure banking portal.</p>
      </button>
      <button onClick={() => setUrl('cybox://email')} className="bg-neutral-700 p-4 rounded-lg hover:bg-blue-700 transition-colors">
        <p className="font-bold">CyberMail</p>
        <p className="text-sm text-neutral-300">Encrypted email service.</p>
      </button>
      <button onClick={() => setUrl('cybox://news')} className="bg-neutral-700 p-4 rounded-lg hover:bg-yellow-700 transition-colors">
        <p className="font-bold">CyberNews</p>
        <p className="text-sm text-neutral-300">Latest tech headlines.</p>
      </button>
      <button onClick={() => setUrl('cybox://search')} className="bg-neutral-700 p-4 rounded-lg hover:bg-purple-700 transition-colors">
        <p className="font-bold">CyberSearch</p>
        <p className="text-sm text-neutral-300">Find what you need.</p>
      </button>
    </div>
  </div>
);
...
export default function BrowserApp() {
  ...
  const reload = () => {
    // This is just a simulation, so we don't need to do anything special
    ...
    <div className="w-full h-full flex flex-col bg-neutral-900 text-white">
      ...
      <div className="flex items-center p-2 bg-neutral-800">
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => navigate('cybox://home')} className="p-2 rounded-full hover:bg-neutral-700"><FaHome /></button>
        </div>
